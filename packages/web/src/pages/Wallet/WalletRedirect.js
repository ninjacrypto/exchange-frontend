...
export class WalletRedirect extends React.PureComponent {
  ...
  renderRedirectOrError() {
    ...
    return (
      'ALL' ?
      <Redirect to={`/wallet/${baseRoute}/${currency}`} /> :
      <Message background="status-error">
        {t(`wallet.redirect.${baseRoute}Error`)}
      </Message>
    )
  }
...
export default withNamespaces()(connect(mapStateToProps)(WalletRedirect));
